...
public class DirectExchange extends AbstractExchange
{
    ...
    protected void onBind(final Binding binding)
    {
        ...
        // When adding new binding with out subscriptions ( When creating a queue from UI) , it does not synch them in the
        //cluster until a subscription created. So , when doing a fail over test with multiple nodes , we need to synch bindings
        //within the cluster. So , here we are synching bindings force fully with out any subscriptions
        try {
            ClusterResourceHolder.getInstance().getCassandraMessageStore().
                    synchBindings(ClusterResourceHolder.getInstance().getVirtualHostConfigSynchronizer());
        } catch (Exception e) {
            _logger.error("Error in synchronizing bindings when adding binding " + binding.toString());
        }
