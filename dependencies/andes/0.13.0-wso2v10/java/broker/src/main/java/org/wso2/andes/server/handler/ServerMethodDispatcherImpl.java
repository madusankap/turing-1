...
public class ServerMethodDispatcherImpl implements MethodDispatcher
{
    ...
    private static final Logger _log = Logger.getLogger(ServerMethodDispatcher.class);
    ...
    public boolean dispatchBasicAck(final BasicAckBody body, final int channelId) throws AMQException
    {
        AndesExecuter.getInstance(ClusterResourceHolder.getInstance().getClusterConfiguration().
            ...
            public void run() {
                try {
                    if (_stateManager.getProtocolSession().getChannel(channelId)!= null) {
                        _basicAckMethodHandler.methodReceived(_stateManager, body, channelId);
                    }else {
                        _log.warn("Unable to find the channel for the channel id "+channelId
                                + " discarding the ack for delivery tag "+ body.getDeliveryTag());
                    }
