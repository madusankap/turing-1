            ...
            synchronized (channelIdString) {
                ...
                while(writtenSize < bodySize)
                {
                    ...
                    if( writtenSize < oldWrittenSize && writtenSize < bodySize) {
                        throw new AMQException("Unexpected Error while getting message content as writtenSize < oldWrittenSize: " +
                                "This might leads to an infinite loop so exiting the loop forcefully");
                    }
                    else if( writtenSize == oldWrittenSize && writtenSize < bodySize) {
                       turn++;
                       if( turn > 5){
                           throw new AMQException("Unexpected Error while getting message content as writtenSize == oldWrittenSize: " +
                                   "This might leads to an infinite loop so exiting the loop forcefully after " + turn + " turns");
                       }else {
                           try {
                               Thread.sleep(2000*turn);
