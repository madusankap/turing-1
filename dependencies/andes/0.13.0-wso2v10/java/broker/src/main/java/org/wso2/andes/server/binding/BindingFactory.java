...
public class BindingFactory
{
    ...
    private boolean makeBinding(String bindingKey, AMQQueue queue, Exchange exchange, Map<String, Object> arguments, boolean restore, boolean force) throws AMQSecurityException, AMQInternalException
    {
        ...
        //check cluster-wide if a durable topic subscription exists with <virtualHost>:clientID (queue name)
        if(exchange.getName().equals("amq.topic") && queue.isDurable()) {

            if(ClusterResourceHolder.getInstance().getCassandraMessageStore()
                    .checkIfDuableSubscriptionAlreadyExists(queue.getName(), bindingKey)) {

                throw new AMQInternalException("Cannot create a binding to a new topic as there is already" +
                        " an existing binding with same subscription ID");
            }
        }
        ...
        /**
         * Never remove the existing mapping (according to JMS spec)
         */
...
/*            if (existingMapping != null)
            {
                if(exchange.getName().equals("amq.topic") && queue.isDurable()) {
                  //in this case also we need to do this and causes no problems as we are subscribing to same topic
    ...
    public void removeBinding(final Binding b) throws AMQSecurityException, AMQInternalException
    {
        ...
        try {
            if(b.getExchange().getName().equalsIgnoreCase("amq.topic")) {
                ...
                messageStore.removeQueueFromTopic(b.getBindingKey(), b.getQueue().getName());
