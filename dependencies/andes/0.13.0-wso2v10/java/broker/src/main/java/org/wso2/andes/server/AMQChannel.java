...
public class AMQChannel implements SessionConfig, AMQSessionModel
{
    ...
    {
          //Need to deny permissions for DLC when an external party attepts to publish information
          //Need to do this since when viewing the contents of the DLC through the browser it creates an echange
        ...
        {       //TODO flow control check
                ...
                try
                {
                    ...
                    Runnable run = new Runnable() {
                        ...
                        public void run() {
                            while (true) {
                                if (incomingMessage.getStoredMessage().getPendingChunkCount().get() == new AtomicInteger(0).get()) {
                                    try {
                                        /**
                                         ...
                                         */
                                        // Store metadata
                                        //todo:hasitha is this required for queue messages?
                                        ...
                                        if (_logger.isDebugEnabled()) {
                                            _logger.debug("TRACING>> Metadata and message written to global queue "
...
//                                    System.out.println("Written meta data for incomingMessage = " + incomingMessage.getStoredMessage().getMessageNumber());
                                    break;
                                }else {
                                    try {
                                        Thread.sleep(100);
                    ...
                    messageRoutingExecutor.submit(run, getChannelId());
             ...
             /*}else {
               _logger.info("Message content for ID " + _currentMessage.getMessageNumber() + " is not persisted to Casandra. Giving up due to timeout");
             }*/
...
//            _messageQueue.addMessageContent(contentChunk.getData().buf());
                                           ...
                                           FieldTable filters, boolean noLocal, boolean exclusive) throws AMQException
    {
        ...
        try {
            ...
            _logger.error("Error while registering subscription", e);
        ...
        DLCQueueUtils.createDLCQueue(queue, getVirtualHost(), ((AMQProtocolEngine) _session).getAuthId().toString());
        return tag;
    }
    ...
    public void close() throws AMQException
    {
        ...
        SequentialThreadPoolExecutor.wait4JobsfromThisChannel2End(this.getId().toString());
