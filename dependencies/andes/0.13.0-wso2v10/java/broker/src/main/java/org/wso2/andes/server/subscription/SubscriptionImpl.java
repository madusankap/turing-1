                        ...
                        try {
                            ...
                            if (((AMQMessage)entry.getMessage()).isTopicMessage() 
                                ...
                                if (traceLog.isTraceEnabled()) {
                                    ...
                                    if(traceLog.isTraceEnabled()) {
                                        traceLog.trace("sent2(" + entry.getMessage().getMessageNumber() + ")"
                                                + new String(buf.array(), 0, readCount) + " channel="+ getChannel().getChannelId() + " deliveryTag=" + deliveryTag);
                                    }
                                }
                                sendToClient(entry, deliveryTag);
                                //_logger.info(" Sending message "+entry.getMessage().getMessageNumber() + " with delivery tag "+deliveryTag + " via channel "+ getChannel().getChannelId());
                                break;
                            } else {
                                    log.warn("message tracker rejected this message from sending msgID:" + entry.getMessage().getMessageNumber() +" Dropping to DLC");
                                /**
                            ...
                            if(i < retryCount -1){
                                //will try again
                                    log.warn("sent4 failed for " + entry.getMessage().getMessageNumber() + " retrying...");
...
//            return false;
        ...
        if (_noLocal)
        {
            ...
            if(publisher != null && publisher.equals(localInstance))
            {
                return false;
            }
