...
public class CassandraDataAccessHelper {
    ...
    private static Log log = LogFactory.getLog(CassandraDataAccessHelper.class);
    ...
    public static Cluster createCluster(String userName, String password, String clusterName,
        ...
        /* reading the tuning configurations for Hector client using andes-config.xml file and adding them here */
        hostConfigurator.setMaxActive(ClusterResourceHolder.getInstance().getClusterConfiguration().getMaxActive());

            // read the hector load balance policy conf and create a new LoadBalancingPolicy accordingly
        String loadBalancePolicyVal = ClusterResourceHolder.getInstance().getClusterConfiguration().getLoadBalancingPolicy();
        LoadBalancingPolicy loadBalancingPolicy;
        if(loadBalancePolicyVal.equalsIgnoreCase("DynamicLoadBalancingPolicy")){
            loadBalancingPolicy = new DynamicLoadBalancingPolicy();
            log.info("Configuring Hector client with DynamicLoadBalancingPolicy" + " for the cluster " );

        } else if (loadBalancePolicyVal.equalsIgnoreCase("LeastActiveBalancingPolicy"))  {
            loadBalancingPolicy = new LeastActiveBalancingPolicy();
            log.info("Configuring Hector client with LeastActiveBalancingPolicy" + " for the cluster " );
        } else {
            loadBalancingPolicy = new RoundRobinBalancingPolicy();
            log.info("Configuring Hector client with RoundRobinBalancingPolicy" + " for the cluster " );
        }
        hostConfigurator.setLoadBalancingPolicy(loadBalancingPolicy);
        hostConfigurator.setMaxWaitTimeWhenExhausted(ClusterResourceHolder.getInstance().getClusterConfiguration().getMaxWaitTimeWhenExhausted());
        hostConfigurator.setUseHostTimeoutTracker(ClusterResourceHolder.getInstance().getClusterConfiguration().getUseHostTimeoutTracker());
        hostConfigurator.setHostTimeoutCounter(ClusterResourceHolder.getInstance().getClusterConfiguration().getHostTimeoutCounter());
        hostConfigurator.setHostTimeoutSuspensionDurationInSeconds(ClusterResourceHolder.getInstance().getClusterConfiguration().getHostTimeoutSuspensionDurationInSeconds());
     ...
     */
    public static void insertCounterColumn(String cfName, String counterRowName,String queueColumn, Keyspace keyspace,long counterValue)
            throws CassandraDataAccessException {
        try {
            Mutator<String> mutator = HFactory.createMutator(keyspace, StringSerializer.get());
            // inserting counter column
            mutator.insertCounter(counterRowName, cfName,HFactory.createCounterColumn(queueColumn, counterValue,StringSerializer.get()));
            mutator.execute();
    ...
    public static void decrementCounter(String columnName ,String columnFamily, String rawID,
                                        Keyspace keyspace, long decrementBy, boolean isCassandraLive)
            throws CassandraDataAccessException {
        try {
            Mutator<String> mutator = HFactory.createMutator(keyspace,StringSerializer.get());
            ...
            if (!isCassandraLive) {
                if (log.isDebugEnabled()) {
                    log.debug("Error while decrementing message counters. Cassandra server is Inaccessible.");
                }
                return;
            }
            mutator.execute();
    ...
    public static ColumnSlice<Long, byte[]> getMessagesFromQueue(String rowName,
                                                                 String columnFamilyName, Keyspace keyspace,
                                                                 long lastProcessedId, long endID,int count) throws CassandraDataAccessException {
        if (keyspace == null) {
            throw new CassandraDataAccessException("Can't access Data , no keyspace provided ");
        }
        ...
        try {
            ...
            sliceQuery.setRange(lastProcessedId + 1 ,endID,false, count);
            sliceQuery.setColumnFamily(columnFamilyName);
...
//            sliceQuery.setRange(startProcessedId + 1 ,lastProcessedId ,false, Integer.MAX_VALUE);
    ...
    public static void addMessageToQueue(String columnFamily , String queue , long messageId ,
       ...
       if (mutator == null) {
        ...
        try {
            mutator.insert(queue.trim(), columnFamily,
                    HFactory.createColumn(messageId, message,longSerializer, bytesArraySerializer));
    ...
    public static void addIntegerByteArrayContentToRaw(String columnFamily,String row,int key,
        ...
        if(traceLog.isTraceEnabled()) {
            traceLog.trace("TRACING>> CDAH-Adding msg content with ID: " +row + " and offset: " + key );
        }
    }
    ...
    public static void deleteLongColumnFromRaw(String columnFamily,String row, long key ,Keyspace keyspace)
        ...
        try {
            Mutator<String> mutator = HFactory.createMutator(keyspace, stringSerializer);
            mutator.delete(row, columnFamily, key, longSerializer);
//            System.out.println("Removed Message meta data with message id = " + key +" from node queue "+ row);
        } catch (Exception e) {
        ...
        try {
            ...
            if (traceLog.isTraceEnabled()) {
                for(String rowname : rows){
                    traceLog.trace("TRACING>> CDAH-Removing message content in Cassandra for MsgID : " + rowname);
                }
            }
        } catch (Exception e) {
            throw new CassandraDataAccessException("Error while deleting data",e);
