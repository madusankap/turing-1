...
public class BasicConsumeMethodHandler implements StateAwareMethodListener<BasicConsumeBody>
{
        ...
        else
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    {
                        if(checkWhetherSubIdIsEmpty(queue)){
                            throw body.getChannelException(AMQConstant.ACCESS_REFUSED,
                                    "Cannot subscribe to queue "
                                            + queue.getNameShortString()
                                            + " as it has an empty subscription id");
                        } else {
    ...
    private boolean checkWhetherSubIdIsEmpty(AMQQueue queue) {
        boolean subIdEmpty = false;
        if(queue.getName().equals("carbon:")){
            subIdEmpty = true;
        }
        return subIdEmpty;
    }
