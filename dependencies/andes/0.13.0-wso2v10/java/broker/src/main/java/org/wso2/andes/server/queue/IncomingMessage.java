...
public class IncomingMessage implements Filterable, InboundMessage, EnqueableMessage, MessageContentSource
{
    ...
    public int addContentBodyFrame(final ContentChunk contentChunk)
            throws AMQException
    {
        _storedMessageHandle.getPendingChunkCount().incrementAndGet();
    ...
    public boolean allContentPersisted(MessageStore messageStore) throws AMQException {

        List chunks = seekChunksMapForMessage((CassandraMessageStore) messageStore);
        int waitRounds = 0;
        while (getBodyCount() != chunks.size()){
                _logger.debug(" Received: " + chunks.size() + " out of Total chunks: " + getBodyCount() + " for MsgID: " + getMessageNumber());
                if( waitRounds < 5){
                    try {
                        Thread.sleep(500);
                        chunks = seekChunksMapForMessage((CassandraMessageStore) messageStore);
                        waitRounds++;
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                } else {
                    _logger.info("Max times waited for chunks breached for Msg: " + getMessageNumber() + " Received: " + chunks.size() + " out of Total chunks: " + getBodyCount());
                    removeMsgEntryFromChunksMap((CassandraMessageStore) messageStore);
                    return false;
                }
        }
            removeMsgEntryFromChunksMap((CassandraMessageStore) messageStore);
    ...
    private void removeMsgEntryFromChunksMap (CassandraMessageStore cassandraMessageStore){
        cassandraMessageStore.getMessageIdToContentChunksMap().removeAll(getMessageNumber());
    }

    private List seekChunksMapForMessage(CassandraMessageStore cassandraMessageStore){
        boolean start = true;
        List chunks = null;
        int turns = 0;
        while (start){
          if( turns < 2) {
              chunks = cassandraMessageStore.getMessageIdToContentChunksMap().get(getMessageNumber());
              if (!chunks.isEmpty()){
                  start = false;
              }else {
                  try {
                      _logger.debug("TRACING >> IM - Waiting to start of message content get written to Cassandra");
                      Thread.sleep(100);
                      turns ++;
                  } catch (InterruptedException e) {
                      e.printStackTrace();
              ...
              _logger.info("IncomingMessage chunks are not added into Casandra for MsgId: " + getMessageNumber());
              start = false;
          }
