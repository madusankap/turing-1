...
public class CassandraManager {
    ...
    public CfDef createCFIfNotFound(KsDef ks, boolean isIncremental) throws MetaException {
        ...
        if (isIncremental) {
            ...
            try {
                ...
                if(indexCfDef == null){
                    indexCfDef =  createIndexColumnFamily(indexKsName);
                }
            } catch (TException e) {
    ...
    private KsDef getOrCreateKeySpace(String ksName) throws MetaException,
            TException, SchemaDisagreementException, InvalidRequestException {
        KsDef ksDef = null;
        try {
            ksDef = getKeyspaceDesc(ksName);
        } catch (NotFoundException e) {
            getOrCreateKeySpace(ksName);
        }
        if (null == ksDef) {
            ksDef = createKeysapce(ksName);
        }
        return ksDef;
    }


    private KsDef createKeysapce(String ksName) throws MetaException,
