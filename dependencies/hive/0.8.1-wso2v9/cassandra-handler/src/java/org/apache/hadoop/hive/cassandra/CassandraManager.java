...
public class CassandraManager {
    ...
    public KsDef createKeyspaceIfNotExisting(String ksName, boolean isIncremental) throws  MetaException {
        ...
        KsDef ksDef = null;
        if(ksDef == null) ksDef= checkAndCreateEventKeyspace(ksName);
        if (ksDef == null) {
    ...
    private KsDef checkAndCreateEventKeyspace(String ksName) throws MetaException {
        ...
        if(ksName != null &&  eventKsName!= null && ksName.equalsIgnoreCase(eventKsName)){
            int replicationFactor = Integer.parseInt(HiveContext.
                    getCurrentContext().getConf().get(CASSANDRA_REPLICATION_FACTOR));
            String strategy = HiveContext.
                    getCurrentContext().getConf().get(CASSANDRA_STRATEGY_CLASS);

            KsDef ksDef = new KsDef();
            ksDef.setName(eventKsName);
    ...
    public KsDef createEventIndexKeyspaceIfNotExists(boolean isIncremental) throws MetaException {
        if(isIncremental) {
            try {
               return getKeyspaceDesc( HiveContext.getCurrentContext().getConf().get(STREAM_INDEX_EVENTS_KEYSPACE));
      ...
      private KsDef createEventIndexKeyspace() throws MetaException {
        String eventIndexKsName = HiveContext.getCurrentContext().getConf().get(STREAM_INDEX_EVENTS_KEYSPACE);
            int replicationFactor = Integer.parseInt(HiveContext.
                    getCurrentContext().getConf().get(CASSANDRA_REPLICATION_FACTOR));
            String strategy = HiveContext.
                    getCurrentContext().getConf().get(CASSANDRA_STRATEGY_CLASS);
