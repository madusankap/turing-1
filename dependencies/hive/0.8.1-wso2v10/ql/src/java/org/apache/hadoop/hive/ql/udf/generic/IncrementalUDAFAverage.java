        ...
        if (parameters[1].getCategory() != ObjectInspector.Category.PRIMITIVE ||
            ...
            throw new UDFArgumentTypeException(1,
                    "Only String type is accepted but "
                            + parameters[1].getTypeName() + " is passed for operation Id.");
        }
...
//      assert (parameters.length == 1);
        ...
        public Object terminateIncrementalEvaluation(AggregationBuffer agg) throws HiveException {
            ...
            long lastCount = 0;
            ByteBuffer sumValue = getLastProcessedValue(cassandraDBHandler, myagg.groupByString,
                    getVariableId(SUM_VARIABLE, myagg.id));
            if (null != sumValue) {
                lastProcessedSum = sumValue.getDouble();
            }
            ByteBuffer countValue = getLastProcessedValue(cassandraDBHandler, myagg.groupByString,
            ...
            if (myagg.count + lastCount == 0) {
                ...
                insertProcessedValue(cassandraDBHandler, myagg.groupByString, getVariableId(SUM_VARIABLE, myagg.id),
                        PrimitiveObjectInspector.PrimitiveCategory.DOUBLE,
                        ByteBufferUtil.bytes(finalSum).array());
                insertProcessedValue(cassandraDBHandler, myagg.groupByString, getVariableId(COUNT_VARIABLE, myagg.id),
