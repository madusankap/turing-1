  ...
  public void write(String keySpace, CassandraProxyClient client, JobConf jc) throws IOException {
      ...
      for (CassandraColumn col : columns) {
          ...
          if (isIncrementalIndex) {
             blockSize  = Integer.parseInt(HiveContext.getCurrentContext().getConf().
                          get(HiveConf.ConfVars.HIVE_CASSANDRA_INCREMENTAL_TIME_BLOCK_SIZE.toString()));
              Long indexCFRowKey = getIndexCFRowKey(timeStamp, blockSize);
          ...
          if (count == batchMutation) {
              ...
              if (isIncrementalIndex) {
                  ...
                  index_mutation_map = getIndexColumnFamilyMutationMap(lastIndexRowTimeCache, key, specialIndexMaps, index_mutation_map, blockSize);
      ...
      if (count > 0) {
          ...
          if (isIncrementalIndex) {
              ...
              String indexKeyspace = HiveContext.getCurrentContext().getConf().get(HiveConf.ConfVars.
                      HIVE_INCREMENTAL_PROCESS_KEYSPACE.toString());
              index_mutation_map = getIndexColumnFamilyMutationMap(lastUpdateColTimeStamp, key, specialIndexMaps, index_mutation_map, blockSize);
              commitChanges(indexKeyspace, client, flevel, index_mutation_map);
          }
      }
  }
