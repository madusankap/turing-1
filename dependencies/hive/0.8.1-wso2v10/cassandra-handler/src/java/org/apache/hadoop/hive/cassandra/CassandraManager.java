...
//                for (String url : urls) {
//                    URI connectionURI = URI.create(url.trim().substring(5));
...
//                }
                ...
                this.host = urlString;
                this.port= Integer.parseInt(dataSource.get(PARAM_PORT));
                this.keyspace = dataSource.get(PARAM_KS);
            } else {
             ...
             if(null != dataSource.get(CarbonDataSourceFetcher.REPLICATION_FACTOR)){
                replicationfactor = Integer.parseInt(dataSource.get(CarbonDataSourceFetcher.REPLICATION_FACTOR));
             }

            if(null != dataSource.get(CarbonDataSourceFetcher.STRATEGY_CLASS)){
                strategyClass =dataSource.get(CarbonDataSourceFetcher.STRATEGY_CLASS);
            }
     ...
     */
    public KsDef getKeyspaceDesc(String ksName) throws NotFoundException, MetaException {
        try {
            if(null ==ksName){
                ksName = keyspace;
            }
            return clientHolder.getProxyConnection().describe_keyspace(ksName);
        } catch (TException e) {
    ...
    public KsDef createKeyspaceIfNotExisting(String ksName, boolean isIncremental) throws  MetaException {
        if(ksName == null) ksName = keyspace;
        KsDef ksDef= checkAndCreateEventKeyspace(ksName);
        if (ksDef == null) {
             ksDef=createKeyspaceWithColumns();
        }
        createEventIndexKeyspaceIfNotExists(isIncremental);
        return ksDef;
    }


    private KsDef checkAndCreateEventKeyspace(String ksName) throws MetaException {
//        String eventKsName = HiveContext.getCurrentContext().getConf().get(STREAM_EVENTS_KEYSPACE);
//        if(ksName != null &&  eventKsName!= null && ksName.equalsIgnoreCase(eventKsName)){

        KsDef ksDef = new KsDef();
        ksDef.setName(ksName);

        LinkedHashMap<String, String> opts = new LinkedHashMap<String, String>();
        opts.put("replication_factor", String.valueOf(replicationfactor));
        ksDef.setStrategy_options(opts);

        ksDef.setReplication_factor(replicationfactor);
        ksDef.setStrategy_class(strategyClass);

        ksDef.addToCf_defs(getCfDef());

        try {
            clientHolder.getProxyConnection().system_add_keyspace(ksDef);
            clientHolder.getProxyConnection().set_keyspace(keyspace);
            return ksDef;
        } catch (InvalidRequestException e) {
            throw new MetaException("Error while creating the keyspace - " + ksName + " in cassandra"
                    + e.getMessage());
    ...
    public KsDef createEventIndexKeyspaceIfNotExists(boolean isIncremental) throws MetaException {
        if(isIncremental) {
            try {
               return getKeyspaceDesc( HiveContext.getCurrentContext().getConf().
                       get(HiveConf.ConfVars.HIVE_INCREMENTAL_PROCESS_KEYSPACE.toString()));
            } catch (NotFoundException e) {
               return createEventIndexKeyspace();
            }
        }
        return null;
    }

      private KsDef createEventIndexKeyspace() throws MetaException {
            String eventIndexKsName = HiveContext.getCurrentContext().getConf().get( HiveContext.getCurrentContext().getConf().
                       get(HiveConf.ConfVars.HIVE_INCREMENTAL_PROCESS_KEYSPACE.toString()));
            int replicationFactor = Integer.parseInt(HiveContext.
                    getCurrentContext().getConf().get(HiveConf.ConfVars.HIVE_INCREMENTAL_PROCESS_REPLICATION_FACTOR.toString()));
            String strategy = HiveContext.
                    getCurrentContext().getConf().get(HiveConf.ConfVars.HIVE_INCREMENTAL_PROCESS_KS_STRATEGY.toString());

            KsDef ksDef = new KsDef();
            ksDef.setName(eventIndexKsName);
            ...
            try {
                ...
                log.error("Error while creating the keyspace - "+eventIndexKsName+" in cassandra , This is due to data hasn't been published");
                return null;
            } catch (SchemaDisagreementException e) {
                 throw new MetaException("Error while creating the keyspace - "+eventIndexKsName+" in cassandra"
                    + e.getMessage());
            } catch (TException e) {
                 throw new MetaException("Error while creating the keyspace - "+eventIndexKsName+" in cassandra"
                    + e.getMessage());
    ...
    private CfDef getColumnIndexFamily(KsDef ks) {
        for (CfDef cf : ks.getCf_defs()) {
            if (cf.getName().equalsIgnoreCase(resolveIndexCFName())) {
                return cf;
            }
        }
        return null;
    ...
    private String resolveIndexCFName() {
        return "event_index_" + columnFamilyName;
    }
    ...
    private CfDef getIndexCFDef(String indexKeyspace){
        CfDef cf = new CfDef();
        cf.setKeyspace(indexKeyspace);
        cf.setName(resolveIndexCFName());

        String defaultValidationClass = CASSANDRA_UTF8_VALIDATOR_TYPE;

        cf.setDefault_validation_class(defaultValidationClass);
        cf.setComparator_type(cassandraValidatorMap.get("BIGINT"));
        cf.setKey_validation_class(CASSANDRA_UTF8_VALIDATOR_TYPE);
        return cf;
    }
     ...
     *
     * @param ks                                                                  HiveConf.ConfVars.HIVE_INCREMENTAL_PROCESS_KEYSPACE.toString());
            try {
                KsDef indexks =  getOrCreateKeySpace(indexKsName);
     * @return
     * @throws MetaException
     */
    public CfDef createCFIfNotFound(KsDef ks, boolean isIncremental) throws MetaException {
        ...
        if (isIncremental) {
            String indexKsName = HiveContext.
                    getCurrentContext().getConf().get(HiveConf.ConfVars.HIVE_INCREMENTAL_PROCESS_KEYSPACE.toString());
            try {
                KsDef indexks =  getKeyspaceDesc(indexKsName);
                CfDef indexCfDef = getColumnIndexFamily(indexks);

                if(indexCfDef == null){
                   createIndexColumnFamily(indexKsName);
                }
            }  catch (NotFoundException e) {
                createEventIndexKeyspace();
            }
    ...
    private CfDef createIndexColumnFamily(String indexKeySpace) throws MetaException {
      CfDef cf = getIndexCFDef(indexKeySpace);
        try {
            clientHolder.getProxyConnection().set_keyspace(indexKeySpace);
    ...
    private Map<String, String> getCassandraColumnValidators() {
        ...
        if (prop != null && schemas != null) {
            ...
            if (!AbstractColumnSerDe.isTransposed(mappings)) {
