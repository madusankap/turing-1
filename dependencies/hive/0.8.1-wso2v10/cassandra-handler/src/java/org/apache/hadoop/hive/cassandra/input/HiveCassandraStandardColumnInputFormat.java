...
public class HiveCassandraStandardColumnInputFormat extends InputFormat<BytesWritable, MapWritable>
    ...
    public RecordReader<BytesWritable, MapWritable> getRecordReader(InputSplit split,
        ...
        if (Boolean.parseBoolean(jobConf.get(HiveConf.ConfVars.HIVE_INCREMENTAL_PROCESS_ENABLE.toString()))) {
        ...
        if (isTransposed || readColIDs.size() == columns.size() || readColIDs.size() == 0) {
            ...
            if (comparatorType != null && !comparatorType.equals("")) {
                try {
                    ...
                    throw new RuntimeException("Syntax is incorrect! "+e.getMessage(), e);
                }
            }
        ...
        try {
            ...
            } else if (isTransposed && tac.getConfiguration().getBoolean(AbstractColumnSerDe.CASSANDRA_ENABLE_WIDEROW_ITERATOR, true)) {
                rr = new CassandraHiveRecordReader(new ColumnFamilyWideRowRecordReader(), isTransposed);
            } else {
                rr = new org.apache.hadoop.hive.cassandra.input.CassandraHiveRecordReader(new ColumnFamilyRowRecordReader(), isTransposed);
            }
