        ...
        if (enclosingElementPropertyName != null) {
            ...
            if (enclosingElementProperty != null) {
                if (enclosingElementProperty instanceof OMElement) {
                    ...
                    try {
                        newCtxWithEnclosingElement.setEnvelope(
                                EIPUtils.encloseWithElement(newCtx.getEnvelope(),
                                                            enclosingElement));
                    } catch (AxisFault axisFault) {
                        handleException("Error occurred while enclosing the result with " +
                                        "enclosing element ", axisFault, newCtx);
                    }
