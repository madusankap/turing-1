...
public class BPELDAOConnectionImpl implements BpelDAOConnection, FilteredInstanceDeletable {
    ...
    public ProcessInstanceDAO getInstanceEagerly(Long iid) {
    	StringBuilder sql = new StringBuilder(" SELECT DISTINCT a FROM ProcessInstanceDAOImpl a LEFT JOIN FETCH a._process ");
		sql.append(" WHERE a._instanceId = :iid ");

		Query query = _em.createQuery(sql.toString());
		query.setParameter("iid", iid);
		List<ProcessInstanceDAO> result = query.getResultList();
	...
	public ScopeDAO getScopeEagerly(Long siidl) {
		...
		List<ScopeDAOImpl> result = query.getResultList();
