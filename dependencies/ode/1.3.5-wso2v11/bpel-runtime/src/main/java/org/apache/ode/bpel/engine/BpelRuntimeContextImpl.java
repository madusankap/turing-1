...
public class BpelRuntimeContextImpl implements BpelRuntimeContext {
    ...
    private static boolean doOneWayAck = false;

    static {
        String ack = System.getProperty("ode.onewayAck");
        if(ack != null && "true".equalsIgnoreCase(ack)) {
            doOneWayAck = true;
        }
    }
    ...
    public String invoke(int aid, PartnerLinkInstance partnerLink, Operation operation, Element outgoingMessage,
        ...
        if (mexDao.getPattern().equals(MessageExchangePattern.REQUEST_ONLY.toString())) {
            if(doOneWayAck) {
                // If one way ack property is set to true, then we have to propagate the message exchange status forward.
                // Otherwise we will simply ignore the message exchange status and set ASYNC status  Note: Custom modification
        ...
        switch (mex.getStatus()) {
            ...
            case ASYNC:
            if(!doOneWayAck) {
                // if one way ack is not set we response to the invocation. Note: Custom modification
                break;
            }
            case RESPONSE:
            case FAULT:
    ...
    private void invocationResponse2(String mexid, InvokeResponseChannel responseChannel) {
        ...
        switch (status) {
            ...
            case ASYNC:
                if(doOneWayAck){
                    // If one way ack property is set, we will respond on the response channel.Does not exist on ode
